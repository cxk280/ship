...
- **A verification-method lesson:** a full-page screenshot initially *masked* a layout bug — the standalone Security Probe page couldn't scroll under the app's `overflow:hidden` root, but a `fullPage` capture renders past the clip and looked fine. It surfaced only when a real user tried to scroll. Interactive checks (or asserting `scrollHeight > clientHeight`) catch what a tall screenshot hides.
