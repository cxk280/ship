...
import { stubWebhooks, stubIssues, stubSprints, stubAudit } from '../../../webhooks/__tests__/test-doubles.js';
...
function v1App(documents: DocumentsPort) {
  const app = express();
  app.use('/api/v1', createV1Router({ bearerAuth, rateLimit: noRateLimit, identity, documents, issues: stubIssues, sprints: stubSprints, webhooks: stubWebhooks, audit: stubAudit }));
  return app;
}
