...
import { stubWebhooks, stubIssues, stubSprints, stubAudit, stubIdempotency, noopBus } from '../../../webhooks/__tests__/test-doubles.js';
...
function v1App() {
  const a = express();
  a.use(express.json());
  a.use('/api/v1', createV1Router({ bearerAuth, rateLimit: noRateLimit, identity: identityAdapter, documents: documentsAdapter, issues: stubIssues, sprints: stubSprints, webhooks: stubWebhooks, audit: stubAudit, idempotency: stubIdempotency }));
  return a;
}
