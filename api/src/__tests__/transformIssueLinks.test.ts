...
const queryMock = (): QueryMock => vi.mocked(pool.query) as unknown as QueryMock;
...
describe('transformIssueLinks', () => {
  ...
  describe('pattern matching and transformation', () => {
    ...
    it('transforms multiple issue references in same text', async () => {
      ...
      expect(nodes.some((n: TestNode) => n.text === '#10' && n.marks)).toBe(true);
      expect(nodes.some((n: TestNode) => n.text === '#20' && n.marks)).toBe(true);
      expect(nodes.some((n: TestNode) => n.text === 'issue #30' && n.marks)).toBe(true);
    });
    ...
    it('deduplicates ticket numbers in query', async () => {
      ...
      const queryArgs = queryMock().mock.calls[0]![1] as unknown[];
  ...
  describe('edge cases', () => {
    ...
    it('transforms existing issues but not non-existent ones', async () => {
      ...
      const link50 = nodes.find((n: TestNode) => n.text === '#50');
      expect(link50?.marks).toBeDefined();

      // #999 should be plain text (no marks)
      const text999 = nodes.find((n: TestNode) => n.text === '#999');
  ...
  describe('nested content structures', () => {
    it('transforms issue links in nested paragraphs', async () => {
      ...
      const link = paragraph.content.find((n: TestNode) => n.text === '#25');
    ...
    it('transforms issue links in blockquotes', async () => {
      ...
      const link = paragraph.content.find((n: TestNode) => n.text === 'issue #77');
      expect(link?.marks).toBeDefined();
    });
  ...
  describe('case variations', () => {
    it('handles "issue #" with various casings', async () => {
      ...
      expect(nodes.some((n: TestNode) => n.text === 'Issue #5' && n.marks)).toBe(true);
      expect(nodes.some((n: TestNode) => n.text === 'ISSUE #6' && n.marks)).toBe(true);
    });
  });
